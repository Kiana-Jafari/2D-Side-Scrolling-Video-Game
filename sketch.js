...
function grantExtraLives(heart)
{
	// Add extra live on the platforms
	if (!heart.touched)
	{
		textSize(32);
		text(heart.live, heart.xPos, heart.yPos);
	}
	
	if (!heart.touched && dist(charX, charY, heart.xPos, heart.yPos) < 20)
	{
		heart.touched = true;
		lives ++;
		bonusSound.play();
...
function startGame()
{
	...
	platforms.push(createPlatforms(2 * charX + 130, floorPos_y - 80, 180));
	platforms.push(createPlatforms(charX + 40, floorPos_y - 80, 180));

	enemies = [];

	enemies.push(new Enemy(-140, floorPos_y + 17, 100));
	enemies.push(new Enemy(-510, floorPos_y + 17, 100));
	enemies.push(new Enemy(940, floorPos_y + 17, 100));
	enemies.push(new Enemy(1500, floorPos_y + 17, 100));
