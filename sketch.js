...
function drawMountains()
{
	for (let i = 0; i < mountains.length; i++)
	{
		...
		triangle(
			...
			mountains[i].x_pos + 20, mountains[i].y_pos - 300,
			mountains[i].x_pos + 100, mountains[i].y_pos);
		...
		triangle(
			mountains[i].x_pos - 100, mountains[i].y_pos,
			mountains[i].x_pos + 5, mountains[i].y_pos - 265,
			mountains[i].x_pos + 100, mountains[i].y_pos); // Offset `y` by a constant to make shadows
	}
}
...
function checkCanyon(t_canyon)
{
	...
	if (
		...
		gameCharX > t_canyon.x_Pos + 20 &&
		gameCharX < t_canyon.x_Pos + t_canyon.width + 10
	)
	{
		isPlummeting = true;
	}
